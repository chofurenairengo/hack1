...
完全版ツリーは **01_foundation.md §4** を参照。
...
| ------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------- | ---------------------------------------------------------------------------- |
| **A（基盤→スライド・管理）**    | Day 1-3：基盤構築 / Day 4〜：スライド作成フロー + Gemini 3 Flash + PPTX + 管理画面 + 被紹介者確認                                               | スライド生成〜本番レンダリング〜お土産PPTX                       | **01_foundation.md** + **02_a_slide.md**                                     |
| **B（アバター）**               | VRM 3D描画 + MediaPipe表情 + アバター同期 + プレゼン画面レイアウト                                                                              | プレゼン中・交流タイム・スタンバイ画面のアバター                 | **03_b_avator.md**                                                           |
| **C（リアルタイム基盤）**       | WebRTC音声 + Realtime（スライド同期・表情同期・スタンプ）+ 交流タイムUI（円卓）                                                                 | イベント当日の配信・同期・エフェクト全般                         | **04_c_realtime.md**                                                         |
| **D（投票・マッチング・事後）** | 投票UI + k-partition 2-opt 実装 + テーブル案内 + 紹介者専用テーブル導出 + マッチ通知 + 推薦フラグ + スタンプ集計 + 1:1チャット + 顔写真合意公開 | 投票から交流テーブル案内、マッチ成立後のチャット・顔写真公開まで | **05_d_voting_matching_epilogue.md** + **06_d_presenter_table_extension.md** |
...
| #   | ファイル名                            | 主な読者                | 内容                                                                                                                                                      |
| --- | ------------------------------------- | ----------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 00  | **00_tech_spec_overview.md**（本書）  | 全員                    | プロジェクト全体像、技術スタック、クリーンアーキテクチャ方針、7ファイルの索引                                                                             |
| 01  | **01_foundation.md**                  | メンバーA、全員（参照） | ディレクトリ構造完全版、DBスキーマ全テーブル、RLS、Server Action 規約、Repository 規約、CI/CD、Git運用、環境変数、引き渡しチェックリスト                  |
| 02  | **02_a_slide.md**                     | メンバーA               | Slide/Deck の Domain・Application・Infrastructure 設計、Gemini 3 Flash 連携、PPTXエクスポート、被紹介者確認、主催者管理画面、内面エピソード誘導UI         |
| 03  | **03_b_avator.md**                    | メンバーB               | VRM 3D描画、MediaPipe 表情トラッキング、ブレンドシェイプマッピング、リップシンク、プレゼン画面レイアウト、非アニメ風VRM選定プロセス、スマホPoC            |
| 04  | **04_c_realtime.md**                  | メンバーC               | WebRTC 音声通話、Supabase Realtime スライド同期・表情同期、スタンプリアクション（エフェクト＋クールタイム）、交流タイム円卓UI、イベント進行ステートマシン |
| 05  | **05_d_voting_matching_epilogue.md**  | メンバーD               | 投票UI、k-partition 2-opt アルゴリズム、テーブル案内、マッチ通知、推薦フラグ、スタンプ集計・表彰、1:1チャット、顔写真相互公開（RLS設計詳細）              |
| 06  | **06_d_presenter_table_extension.md** | メンバーD、B/C（参照）  | 紹介者専用テーブル拡張。被紹介者テーブルから紹介者テーブルを後処理で導出する一次情報                                                                      |

読む順番：

- **最初にAが読む**：00 → 01（を書きながら固める） → 02
- **B/C/D は4/16以降に**：00 → 01（§4〜§16 は必読、他はリファレンス扱い） → 自分の担当ファイル（D は 05 → 06 の順に確認）

---

## 7. 命名・規約の横断ルール（全メンバー共通）

詳細は **01_foundation.md §12〜§15** に記載。ここでは骨子のみ：
...
- 本技術仕様書 `01_foundation.md`（共通基盤・DB・CI/CD）
- 本技術仕様書 `02_a_slide.md`
- 本技術仕様書 `03_b_avator.md`
- 本技術仕様書 `04_c_realtime.md`
- 本技術仕様書 `05_d_voting_matching_epilogue.md`
- 本技術仕様書 `06_d_presenter_table_extension.md`
- Clean Architecture（Robert C. Martin, 2017）— 3層分割の依存ルール
- DDD戦術的設計（Evans, 2003）— エンティティ / 値オブジェクト / リポジトリ の用語定義
- Next.js 16 App Router 公式ドキュメント（Server Action / Route Handler の挙動）
