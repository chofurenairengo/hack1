...
**`event_tables`**：交流タイムのテーブル割り振り結果

| カラム          | 型          | 制約 / 備考                                                   |
| --------------- | ----------- | ------------------------------------------------------------- |
| id              | uuid        | PK                                                            |
| event_id        | uuid        | FK→events.id                                                  |
| table_number    | int         | イベント内のテーブル番号                                      |
| seat_count      | int         | CHECK 3〜5。5 人卓は被紹介者 N=5 のみアプリケーション側で許可 |
| table_type      | text        | CHECK 'presentee','presenter'（06_d の追加候補）              |
| paired_table_id | uuid        | FK→event_tables.id（06_d の追加候補）                         |
| created_at      | timestamptz |                                                               |
...
| -------- | ---- | ------------------ |
| id       | uuid | PK                 |
| table_id | uuid | FK→event_tables.id |
| user_id  | uuid | FK→users.id        |
...
| `Gender`            | `src/domain/user/value-objects/gender.vo.ts`               | `female` / `male`                                                                           | PR #70 で2値化済み。DB の `users.gender` CHECK 制約とも一致させる                                          |
...
| `FullTableAssignmentPlan` | `src/domain/matching/types.ts`                               | `tablePairs: TablePair[]` / `presenteeScore: number`                                       | `ComputeMatching` の保存・返却単位。06_d §4.1 に既載               |
...
上記の型定義は **B（PM）が Phase 0 で確定** し main にマージします。`src/types/api.ts` / `src/types/db.ts` の管轄は B（PM）です。Phase 1 以降の変更は仕様に対する例外であるため、PR 内で影響レーンを明記します：

- Payload 型の**具体的フィールド追加・変更**は、各担当（A/B/C/D）が PR で提案 → 影響レーンを明記してマージ
- enum の値変更は PR 内で変更理由と影響レーンを明記する
- Phase 0 終了時点で、各メンバーが `import { SlideSyncPayload, ExpressionPayload, ... } from '@/domain/...'` して型エラーなくコンパイルできる状態を作る
...
基盤引き渡し後、メンバーAは **02_a_slide.md** に従って以下を担当します：
...
- 本技術仕様書 `00_概要.md` §4（クリーンアーキテクチャ方針）§5（開発フェーズ）§6（7ファイル構成）
- 本技術仕様書 `02_a_slide.md` §2（Slide 関連テーブル運用）
- 本技術仕様書 `03_b_avator.md` §9（VRMアセット配置先）
