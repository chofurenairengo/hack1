...
- 型は `src/types/api.ts` (A 管轄) に定義 → A と事前合意
...
export function useEventPhase(eventId: string): { phase: Phase; transition: (next: Phase) => Promise<void> }

// src/hooks/useSlideSync.ts
export function useSlideSync(eventId: string): { currentPage: number; setPage: (page: number) => void }

// src/hooks/useStampBroadcast.ts
export function useStampBroadcast(eventId: string): { send: (kind: StampKind) => void; recent: Stamp[] }

// src/hooks/useAvatarSync.ts
export function useAvatarSync(eventId: string): { emit: (state: AvatarState) => void; others: Record<UserId, AvatarState> }
```
