---
description: '4 人並列開発のレーン境界 (A=スライド/管理, B=PM/VRM, C=Realtime, D=投票/マッチング) — 越境編集禁止 / クロスメンバー型契約は B（PM）管轄'
globs: ['src/**/*', 'supabase/**/*']
alwaysApply: true
---
...
| -------- | ----------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **A**    | スライド・Gemini・pptxgenjs・管理画面・DB基盤         | `src/domain/slide/`, `src/application/slide/`, `src/infrastructure/ai/gemini/`, `src/infrastructure/pptx/`, `src/app/(dashboard)/admin/**`, `supabase/migrations/`                                              |
| **B**    | PM・VRM アバター・MediaPipe・クロスメンバー型         | `src/infrastructure/avatar/`, `src/components/features/avatar/`, `src/types/api.ts`, `src/types/db.ts`                                                                                                          |
...
- `src/types/api.ts` / `src/types/db.ts` (クロスメンバー型契約) — **B（PM）管轄**。変更は B の承認が必要
...
- クロスメンバー型 (`src/types/api.ts`, `src/types/db.ts`) を B（PM）合意なく変更しない
