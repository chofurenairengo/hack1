...
6. **相互投票 rank 合計がソフト最優先 (辞書式第1位)**。男女 2:2 は辞書式第2位。ハード制約ではない。同性紹介ペアも許容。
7. **紹介者は交流タイムの卓に完全不参加** (ラウンジ/休憩エリア)。同テーブル禁止制約は不参加により自動充足される。マッチング計算の入力・対象は被紹介者のみ。イベント参加は紹介者+被紹介者の 2 人組必須。
...
11. `users.gender` の値は `female` / `male` のみ。他の値を導入しない。
...
19. **被紹介者のドタキャン動的リバランシング**: 被紹介者の参加人数変動時にソフト目的関数を再計算する。紹介者は卓不参加のため被紹介者のみが計算対象。
